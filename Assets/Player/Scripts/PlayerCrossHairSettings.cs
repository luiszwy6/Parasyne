...
public class PlayerCrossHairSettings : MonoBehaviour
{
    ...
    [Header("TPS Aim Sampling (Mouse Raycast)")]
    public LayerMask tpsMouseAimLayers = 0;
    ...
    [Header("Recoil (shared by top-down & TPS)")]
    public float recoilReturnSpeed = 18f;
    public float recoilUpMetersPerShot = 0.12f;
    public float recoilBackMetersPerShot = 0.00f;
    ...
    )
    {
        ...
        if (usingMouseScheme && aimCamera != null && Mouse.current != null)
        {
            ...
            LayerMask activeMouseAimLayers = tpsMode ? tpsMouseAimLayers : mouseAimLayers;

            if (activeMouseAimLayers.value != 0 &&
                Physics.Raycast(ray, out RaycastHit hit, mouseRayMaxDistance, activeMouseAimLayers, mouseRayTriggerInteraction))
            {
    ...
    {
        AddRecoil(recoilUpMetersPerShot, recoilBackMetersPerShot);
    }
