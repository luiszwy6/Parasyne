...
public class EnemyCheckState : EnemyState
{
    ...
    public override void Enter()
    {
        ...
        {
            enemy.animator.SetBool("IsCheck", true);
            enemy.animator.SetBool("IsAlert", false);
        
            // since aware is jsut transition, so turn it off imediately when entering check
    ...
    public override void Update()
    {
        ...
        if (forceAlert || score >= enemy.scoreToAlert)
        {
            ...
            enemy.stateMachine.ChangeState(new EnemySurpriseState(enemy));
            return;
        }
